...
export const logos = [
  ...
  { id: 19, src: "/api/imgs/brands/trustbrands19.png", alt: "Company 19" },
];
      ...
      <div className="relative flex">
        ...
        <Marquee repeat={2} className="flex gap-4 w-max [--duration:40s]">
          {logos.map((logo) => (
            <div
              key={logo.id}
              className={`flex-shrink-0 w-36 ${logo.id === 17 ? "sm:w-28" : "sm:w-44"}  h-24 sm:h-28 rounded-2xl flex items-center justify-center mx-2 sm:mx-4 transform-gpu`}
            >
              <img
                src={logo.src}
                alt={logo.alt}
                ...
                loading="eager"
                decoding="async"
                className={`${logo.id === 15 ? "h-28" : logo.id === 17 ? "h-[1000px]! max-w-[190px]!" : logo.id === 18 ? "h-37" : "h-48"} w-auto object-contain group-hover:opacity-80 transition-opacity duration-300`}
              />
            </div>
          ))}
        </Marquee>
